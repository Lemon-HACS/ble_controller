...
디바이스의 실제 ON/OFF 상태를 HA에 반영하려면 추가로 설정하세요.
...
> **상태 조회 커맨드**: 연결 직후 디바이스에 "지금 상태가 뭐야?" 라고 묻는 명령입니다. Keep Alive 활성화 시 재연결할 때마다 자동 전송되어, HA 재시작 후에도 스위치 상태가 즉시 반영됩니다.

### Keep Alive 설정 (권장)

이 디바이스는 BLE 연결이 유휴 상태면 비교적 빠르게 끊깁니다. Keep Alive를 활성화하고 주기를 짧게 설정하세요.

| 항목 | 값 |
|------|-----|
| Keep Alive | ON |
| Keepalive 주기 | **5초** (10초에서는 끊김 발생, 5초 권장) |
...
- **Write 방식**: Write Without Response를 사용합니다. BLE Controller 설정에서 Write With Response를 반드시 OFF로 두세요. ON으로 하면 디바이스가 ACK를 보내지 않아 명령이 타임아웃됩니다.
- **Keep Alive 권장**: 이 디바이스는 유휴 연결을 빠르게 끊으므로 Keep Alive + Keepalive 주기 5초를 권장합니다.
